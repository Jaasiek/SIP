...
def public_transport_stops_info_get(path) -> None:
    with urllib.request.urlopen(
        "https://api.um.warszawa.pl/api/action/public_transport_dictionary/?apikey=12f6c8fa-cc23-4405-9c42-30071dbcbfba"
    ) as url:
        data = json.load(url)
        with open(path, "w") as file:
            file.write(json.dumps(data))
...
if __name__ == "__main__":
    try:
        ...
        public_transport_stops_info_get(stops_info_path)
        naming_stops(
            routes_path=routes_path,
            stops_path=stops_path,
            stops_info_path=stops_info_path,
