...
function NextStop(data) {
  ...
  if (
    ...
    const audio = new Audio(
      `http://192.168.88.187:5000/next_stop_announcement/${encodeURIComponent(
        data.next_stop
      )}.mp3`
    );
    ...
    if (shortened_course && streets.length <= 10) {
      setTimeout(() => {
        const shortened_course_audio = new Audio(
          `http://192.168.88.187:5000/shortened_course/${encodeURIComponent(
            direction
          )}.mp3`
        );
...
function CurrentStop(data) {
  ...
  if (data.stop_type == "4") {
    const audio = new Audio(
      `http://192.168.88.187:5000/last_stop_announcement/${encodeURIComponent(
        data.current_stop
      )}.mp3`
    );
    ...
    const audio = new Audio(
      `http://192.168.88.187:5000/current_stop_announcement/${encodeURIComponent(
        data.current_stop
      )}.mp3`
    );
